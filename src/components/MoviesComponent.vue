      ...
      <div v-if="filteredSeries.length">
        <div v-for="serie in filteredSeries" :key="serie.id" class="serie">
          <router-link :to="{ name: 'seriesDetail', params: { id: serie.id } }">
            <div class="tooltip">
              <img :src="getPosterPath(serie.poster_path)" :alt="serie.name" />
...
const getPosterPath = (path: string | null) => {
  return path ? `https://image.tmdb.org/t/p/w500${path}` : '/images/default-poster.jpg'
}
...
onMounted(async () => {
  ...
  filteredSeries.value = Array.from(new Set(seriesStore.series.map(serie => serie.id)))
    .map(id => seriesStore.series.find(serie => serie.id === id))
    .filter(serie => serie !== undefined) as Series[]
  loading.value = false
})
</script>
